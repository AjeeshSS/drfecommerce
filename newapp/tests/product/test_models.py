...
from newapp.models import Category, Product, ProductLine

pytestmark = pytest.mark.django_db


class TestCatagoryModel:
    """Test category model"""
    ...
    def test_name_unique_field(self, category_factory):
        category_factory(name="test_category")
    ...
    def test_return_category_active_only_true(seld, category_factory):
        category_factory(is_active=True)
        category_factory(is_active=False)
        qs = Category.objects.is_active().count()
        assert qs == 1

    def test_return_category_count(seld, category_factory):
        category_factory(is_active=True)
        category_factory(is_active=False)
...
class TestProductModel:
    ...
    def test_return_product_active_only_true(seld, product_factory):
        product_factory(is_active=True)
        product_factory(is_active=False)
        qs = Product.objects.is_active().count()
        assert qs == 1

    def test_return_product_count(seld, product_factory):
        product_factory(is_active=True)
        product_factory(is_active=False)
...
class TestProductLineModel:
    ...
    def test_fk_product_type_on_delete_protect(self, product_line_factory, product_type_factory):
    ...
    def test_duplicate_order_values(self, product_line_factory, product_factory):
        """test case for clean fields in productline model"""
        obj = product_factory()
    ...
    def test_field_sku_max_digits(self, product_line_factory):
        sku = "x" * 11
        with pytest.raises(ValidationError):
            product_line_factory(sku=sku)

    def test_is_digital_false_default(self, product_line_factory):
        obj = product_line_factory(is_active=False)
        assert obj.is_active is False

    def test_fk_product_on_delete_protest(self, product_line_factory, product_factory):
...
class TestProductImageModel:
    """Test cases for ProductImageModel"""
    ...
    def test_alternativ_text_field_length(self, product_image_factory):
...
class TestAttributeModel:
    def test_str_methof(self, attribute_factory):
